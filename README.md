...
tree is walked exactly as the circuit walks it and must reach the same root.

The Rust proof service drives the pinned toolchain rather than linking Barretenberg, because those
binaries produce the verifier the contract deploys and a second proving implementation is a second
thing that can disagree with it. Private inputs have no `Serialize` impl at all, so they cannot
reach a response body even by accident; tests assert directly that no response and no error message
contains the secret.

Commitments and spent nullifiers are indexed into Postgres and served over `/v1/zk`, and **nothing
indexed records a sender**. The chain reveals one, so storing it leaks nothing new in principle —
but a table putting an inserter beside a spender turns correlation from log archaeology into a
single join, and that difficulty is the anonymity the module provides. `/v1/zk/anonymity-set`
serves the leaf count, because a membership proof is only as private as the number of members. The depth constant
...
Current: **367 contract tests**, **21 circuit tests**, **26 zk service tests**, **62 Solana tests**, **112 frontend tests**, 16 backend packages, **44 end-to-end tests**, Slither clean.
...
Solana work needs the Solana CLI and Anchor at the versions pinned in
[`solana/Anchor.toml`](solana/Anchor.toml); verify with `make solana-toolchain-check`.
